...
// saveGlobals saves and restores all package-level mutable state.
func saveGlobals(t *testing.T) {
	t.Helper()
	origNow := nowFunc
	origOpenMeteo := openMeteoBaseURL
	origGeocode := geocodeBaseURL
	origNWS := nwsBaseURL
	origNominatim := nominatimBaseURL
	origTemplates := templates
	t.Cleanup(func() {
		nowFunc = origNow
		openMeteoBaseURL = origOpenMeteo
		geocodeBaseURL = origGeocode
		nwsBaseURL = origNWS
		nominatimBaseURL = origNominatim
...
// setupAllMocks configures all base URLs and returns cleanup.
func setupAllMocks(t *testing.T) {
	...
	nwsBaseURL = nwsSrv.URL
	openMeteoBaseURL = meteoSrv.URL
	geocodeBaseURL = geocodeSrv.URL
...
func TestFetchForecast(t *testing.T) {
	...
	data, err := fetchForecast("40.71", "-74.01")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFetchForecast_Error(t *testing.T) {
	...
	_, err := fetchForecast("40.71", "-74.01")
	if err == nil {
...
func TestFetchGeocode(t *testing.T) {
	...
	data, err := fetchGeocode("Portland")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFetchNWSPoints(t *testing.T) {
	...
	data, err := fetchNWSPoints("40.71", "-74.01")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFetchNWSPoints_Error(t *testing.T) {
	...
	_, err := fetchNWSPoints("40.71", "-74.01")
	if err == nil {
		t.Error("expected error")
	}
}
...
func TestFetchNWSGridpoint(t *testing.T) {
	...
	data, err := fetchNWSGridpoint("OKX", 33, 37)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFetchNWSGridpoint_Error(t *testing.T) {
	...
	_, err := fetchNWSGridpoint("OKX", 33, 37)
	if err == nil {
		t.Error("expected error")
	}
}
...
func TestReverseGeocode(t *testing.T) {
	...
	t.Run("city", func(t *testing.T) {
		...
		name, err := reverseGeocode("40.71", "-74.01")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if name != "New York" {
	...
	t.Run("network error", func(t *testing.T) {
		nominatimBaseURL = "http://localhost:1"
		_, err := reverseGeocode("40.71", "-74.01")
		if err == nil {
			t.Error("expected error")
		}
	})
...
func TestTransformNWSHourly(t *testing.T) {
	...
	rows := transformNWSHourly(periods, "America/New_York")
...
func TestTransformNWSHourly_InvalidTimezone(t *testing.T) {
	...
	rows := transformNWSHourly(periods, "Bad/Zone")
	if len(rows) != 1 {
...
func TestTransformNWSDaily(t *testing.T) {
	...
	rows, globalLow, globalHigh := transformNWSDaily(periods, "America/New_York")
...
func TestTransformNWSDaily_NightOnly(t *testing.T) {
	...
	rows, _, _ := transformNWSDaily(periods, "America/New_York")
	if len(rows) != 1 {
		t.Fatalf("expected 1 row, got %d", len(rows))
	}
...
func TestTransformNWSDaily_InvalidTimezone(t *testing.T) {
	...
	periods := makeNWSDailyPeriods()
	rows, _, _ := transformNWSDaily(periods, "Bad/Zone")
	if len(rows) == 0 {
		t.Error("expected rows with invalid timezone fallback to UTC")
	}
}
...
func TestTransformNWSDaily_ZeroTempRange(t *testing.T) {
	...
	rows, _, _ := transformNWSDaily(periods, "UTC")
	if len(rows) != 1 {
		t.Fatalf("expected 1 row, got %d", len(rows))
	}
...
func TestCalcNWSPrecipWithQPF(t *testing.T) {
	...
	entries, total := calcNWSPrecipWithQPF(days, precipByDay, "UTC")
	if len(entries) != 2 {
...
func TestCalcNWSPrecipWithQPF_InvalidTimezone(t *testing.T) {
	...
	entries, _ := calcNWSPrecipWithQPF(days, precipByDay, "Bad/Zone")
	if len(entries) != 1 {
...
func TestTransformHourlyForDay(t *testing.T) {
	...
	data := makeOpenMeteoForecastResponse()
	rows := transformHourlyForDay(&data, "2026-02-08")

	if len(rows) == 0 {
		t.Fatal("expected rows for target date")
	}
	// Should skip hours before 14:00 on today
	for _, r := range rows {
		_ = r // rows are validated by count and IsNow below
	}
...
func TestTransformHourlyForDay_FutureDate(t *testing.T) {
	...
	data := makeOpenMeteoForecastResponse()
	rows := transformHourlyForDay(&data, "2026-02-09")
...
func TestTransformNWSHourlyForDay(t *testing.T) {
	...
	periods := makeNWSHourlyPeriods()
	rows := transformNWSHourlyForDay(periods, "America/New_York", "2026-02-08")
...
func TestTransformNWSHourlyForDay_InvalidTimezone(t *testing.T) {
	...
	rows := transformNWSHourlyForDay(periods, "Bad/Zone", "2026-02-08")
	if len(rows) != 1 {
...
func TestHandleForecast_OpenMeteoPathNoName(t *testing.T) {
	...
	body := w.Body.String()
	if !strings.Contains(body, "40.71") || !strings.Contains(body, "-74.01") {
		// The mock returns lat=40.71 lon=-74.01 so the auto-name uses those
	}
}
...
func TestHandleForecast_OpenMeteoError(t *testing.T) {
	...
	defer failSrv.Close()
	nwsBaseURL = failSrv.URL
	openMeteoBaseURL = failSrv.URL
